...
export async function getPairingHistory(vehicleId?: string, assetId?: string): Promise<PairingHistory[]> {
  await new Promise(resolve => setTimeout(resolve, 100));
...
export async function createPairing(pairing: Omit<VehiclePairing, "id" | "pairedAt">): Promise<VehiclePairing> {
...
export async function updateVehicleLocation(vehicleId: string, location: { lat: number; lng: number }): Promise<Vehicle> {
  await new Promise(resolve => setTimeout(resolve, 100));
