...
def downgrade():
    ...
    op.execute("SELECT remove_retention_policy('estimated_locations', if_exists => TRUE);")
    op.execute("SELECT remove_retention_policy('observations', if_exists => TRUE);")
    
    # Remove compression policies
    op.execute("SELECT remove_compression_policy('estimated_locations', if_exists => TRUE);")
    op.execute("SELECT remove_compression_policy('observations', if_exists => TRUE);")
    
    # Convert hypertables back to regular tables
    op.execute("SELECT * FROM timescaledb_information.hypertables WHERE hypertable_name = 'audit_logs';")
    op.execute("SELECT * FROM timescaledb_information.hypertables WHERE hypertable_name = 'alerts';")
    op.execute("SELECT * FROM timescaledb_information.hypertables WHERE hypertable_name = 'estimated_locations';")
    op.execute("SELECT * FROM timescaledb_information.hypertables WHERE hypertable_name = 'observations';")
