...
revision = "003_create_all_tables"
...
def upgrade() -> None:
    ...
    op.create_index(
        "idx_user_org_email", "users", ["organization_id", "email"], unique=False
    )
    op.create_index(
        "idx_user_org_username", "users", ["organization_id", "username"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column("radius", sa.Integer(), nullable=True),
        sa.Column(
            "coordinates", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column("site_id", sa.String(), nullable=True),
