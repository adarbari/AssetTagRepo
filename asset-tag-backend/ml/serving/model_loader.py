...
class ModelLoader:
    ...
    async def load_model(self, model_name: str, model_type: str = "sklearn") -> Optional[Any]:
        ...
        try:
            async with self.load_lock:
                ...
                cached_model = await cache.get_with_strategy("ml_predictions", asset_id=model_name)
                ...
                # Load from MLflow
                model_uri = self.mlflow_client.get_latest_model("anomaly_detection", model_name)
                if not model_uri:
                ...
                await cache.set_with_strategy("ml_predictions", model, asset_id=model_name)
    ...
    async def get_model(self, model_name: str, model_type: str = "sklearn") -> Optional[Any]:
        ...
        while self.running:
            try:
                ...
                if self.running:
                    ...
                    total = len(results)
                    logger.info(f"Model refresh completed: {refreshed}/{total} models refreshed")
