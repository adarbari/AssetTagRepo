...
async def predict_anomaly(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error predicting anomaly: {str(e)}"
        )
...
async def predict_location(
    ...
    future_minutes: int = Query(
        30, ge=1, le=1440, description="Minutes into the future to predict"
    ),
    inference_engine: InferenceEngine = Depends(get_inference_engine),
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error predicting location: {str(e)}"
        )
...
async def predict_battery_degradation(
    ...
    try:
        result = await inference_engine.predict_battery_degradation(
            asset_id, days_ahead
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error predicting battery: {str(e)}"
        )
...
async def batch_predictions(
    ...
    prediction_type: str = Query(
        "anomaly", description="Type of prediction: anomaly, location, battery"
    ),
    inference_engine: InferenceEngine = Depends(get_inference_engine),
):
    ...
    try:
        ...
        if len(asset_id_list) > 50:
            raise HTTPException(
                status_code=400, detail="Maximum 50 assets per batch request"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error in batch predictions: {str(e)}"
        )


@router.get("/models/status")
async def get_model_status(model_loader: ModelLoader = Depends(get_model_loader)):
...
async def refresh_models(
    model_name: Optional[str] = Query(
        None, description="Specific model to refresh, or all if not specified"
    ),
...
async def preload_models(
    model_names: str = Query(
        ..., description="Comma-separated list of model names to preload"
    ),
    model_loader: ModelLoader = Depends(get_model_loader),
):
    ...
    try:
        ...
        if len(model_name_list) > 10:
            raise HTTPException(
                status_code=400, detail="Maximum 10 models per preload request"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error preloading models: {str(e)}"
        )


@router.delete("/models/{model_name}")
async def unload_model(
    model_name: str, model_loader: ModelLoader = Depends(get_model_loader)
):
