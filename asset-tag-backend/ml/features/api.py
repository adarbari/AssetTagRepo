...
async def invalidate_features(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error invalidating features: {str(e)}")
...
async def get_anomaly_score(
    ...
    try:
        ...
        if not features or not baseline:
            raise HTTPException(status_code=404, detail="Features or baseline not found")
        ...
        raise HTTPException(status_code=500, detail=f"Error calculating anomaly score: {str(e)}")
...
async def get_batch_features(
    ...
    try:
        ...
        if len(asset_id_list) > 100:
            raise HTTPException(status_code=400, detail="Maximum 100 assets per batch request")
        ...
        raise HTTPException(status_code=500, detail=f"Error getting batch features: {str(e)}")


async def _calculate_anomaly_score(features: FeatureVector, baseline: AssetBaseline) -> float:
    ...
    try:
        ...
        if 'avg_battery' in features.features:
            battery_diff = abs(features.features['avg_battery'] - 80.0)  # Assume 80% is normal
            score += min(battery_diff / 50.0, 1.0)  # Normalize by 50%
            feature_count += 1
        
        # Temperature anomaly
        if 'avg_temperature' in features.features:
            temp_diff = abs(features.features['avg_temperature'] - baseline.avg_temperature)
            score += min(temp_diff / 10.0, 1.0)  # Normalize by 10°C
            feature_count += 1
        
        # Confidence anomaly
        if 'current_confidence' in features.features:
            conf_diff = abs(features.features['current_confidence'] - baseline.typical_confidence)
            score += min(conf_diff / 0.5, 1.0)  # Normalize by 0.5
            feature_count += 1
        
        # Movement anomaly
        if 'current_speed' in features.features and baseline.typical_movement_pattern:
            speed_diff = abs(features.features['current_speed'] - baseline.typical_movement_pattern.get('avg_speed', 0))
