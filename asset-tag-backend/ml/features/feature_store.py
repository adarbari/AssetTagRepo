...
class FeatureStore:
    ...
    async def get_features(self, asset_id: str) -> Optional[FeatureVector]:
        ...
        try:
            ...
            cached_features = await cache.get_with_strategy("asset_features", asset_id=asset_id)
            if cached_features:
            ...
            if features:
                # Cache for 5 minutes
                await cache.set_with_strategy("asset_features", features.to_dict(), asset_id=asset_id)
    ...
    async def get_baseline(self, asset_id: str) -> Optional[AssetBaseline]:
        ...
        try:
            ...
            cached_baseline = await cache.get_with_strategy("asset_baseline", asset_id=asset_id)
            if cached_baseline:
            ...
            if baseline:
                # Cache for 24 hours
                await cache.set_with_strategy("asset_baseline", baseline.to_dict(), asset_id=asset_id)
    ...
    async def _compute_real_time_features(self, asset_id: str) -> Optional[FeatureVector]:
        ...
        try:
            ...
            # Battery features
            battery_values = [obs.battery_level for obs in observations if obs.battery_level is not None]
            if battery_values:
                features['avg_battery'] = sum(battery_values) / len(battery_values)
                features['battery_trend'] = self._calculate_trend([obs.battery_level for obs in observations if obs.battery_level is not None])
            
            # Temperature features
            temp_values = [obs.temperature for obs in observations if obs.temperature is not None]
            if temp_values:
            ...
            if location:
                ...
                features['current_speed'] = float(location.speed) if location.speed else 0.0
                features['current_bearing'] = float(location.bearing) if location.bearing else 0.0
            ...
            if len(observations) > 1:
                time_span = (observations[0].observed_at - observations[-1].observed_at).total_seconds()
                features['observation_rate'] = len(observations) / max(time_span / 60, 1)  # per minute
    ...
    async def _compute_baseline_features(self, asset_id: str) -> Optional[AssetBaseline]:
        ...
        try:
            ...
            # Build baseline
            avg_rssi_per_gateway = {str(stat.gateway_id): float(stat.avg_rssi) for stat in gateway_stats}
            
            typical_movement_pattern = {
                "avg_distance": float(movement_stats.avg_distance) if movement_stats.avg_distance else 0.0,
                "avg_speed": float(movement_stats.avg_speed) if movement_stats.avg_speed else 0.0,
                "location_count": int(movement_stats.location_count) if movement_stats.location_count else 0
            }
            
            # Calculate battery drain rate (simplified)
            battery_values = [stat.avg_battery for stat in gateway_stats if stat.avg_battery is not None]
            normal_battery_drain_rate = 0.1  # Default 10% per day
            
            avg_temperature = sum(stat.avg_temperature for stat in gateway_stats if stat.avg_temperature is not None) / len(gateway_stats) if gateway_stats else 20.0
            ...
            return AssetBaseline(
                ...
                typical_confidence=float(avg_confidence.avg_confidence) if avg_confidence and avg_confidence.avg_confidence else 0.8,
    ...
    async def store_features(self, feature_vector: FeatureVector) -> bool:
        ...
        try:
            ...
            cache = await self._get_cache()
            await cache.set_with_strategy("asset_features", feature_vector.to_dict(), asset_id=feature_vector.asset_id)
    ...
    async def get_feature_history(self, asset_id: str, hours: int = 24) -> List[FeatureVector]:
