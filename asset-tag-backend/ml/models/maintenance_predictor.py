...
class MaintenancePredictor:
    ...
    def prepare_features(
        self, asset_data: Dict, historical_data: List[Dict]
    ) -> pd.DataFrame:
    ...
    def predict(self, asset_data: Dict, historical_data: List[Dict]) -> Dict:
        ...
        try:
            ...
            predicted_date = self._predict_maintenance_date(
                features.iloc[0], maintenance_type
            )
    ...
    def _determine_urgency(self, features: pd.Series) -> str:
        ...
        if features["error_count"] > 5 or features["temperature_max"] > 40:
            ...
            features["days_since_maintenance"] > 180
            or features["battery_cycles"] > 1000
        ):
            return "medium"
        else:
            return "low"

    def _predict_maintenance_date(
        self, features: pd.Series, maintenance_type: str
    ) -> datetime:
        """Predict when maintenance will be needed"""
        base_days = {"emergency": 1, "corrective": 7, "preventive": 30, "scheduled": 90}
