...
class LocationPredictor:
    ...
    def __init__(self):
        ...
        self.feature_columns = [
            'hour_of_day', 'day_of_week', 'month', 'rssi_avg', 'rssi_std',
            'gateway_count', 'distance_from_previous', 'time_since_last_update',
            'battery_level', 'temperature', 'movement_speed'
        ]
    ...
    def prepare_features(self, historical_data: List[Dict]) -> pd.DataFrame:
        ...
        try:
            ...
            df['time_since_last_update'] = df['timestamp'].diff().dt.total_seconds().fillna(0)
            df['movement_speed'] = df['distance_from_previous'] / (df['time_since_last_update'] + 1)
    ...
    def predict(self, features: pd.DataFrame, steps_ahead: int = 1) -> List[Dict]:
        ...
        try:
            ...
            for step in range(steps_ahead):
                ...
                lat_confidence = max(0.1, 1.0 - abs(lat_pred - current_features[0][0]) * 0.01)
                lng_confidence = max(0.1, 1.0 - abs(lng_pred - current_features[0][1]) * 0.01)
