...
from modules.notifications.models import NotificationConfig, NotificationLog, NotificationTemplate
from modules.notifications.schemas import (
    NotificationConfigCreate, NotificationConfigUpdate, NotificationConfigResponse,
    NotificationLogResponse, NotificationTemplateCreate, NotificationTemplateUpdate,
    NotificationTemplateResponse, NotificationTestRequest, NotificationTestResponse,
    NotificationStats
)
...
async def update_notification_config(
    ...
    try:
        result = await db.execute(
            select(NotificationConfig).where(
                and_(NotificationConfig.id == config_id, NotificationConfig.deleted_at.is_(None))
            )
        )
        config = result.scalar_one_or_none()
...
async def get_notification_stats(
    ...
    try:
        ...
        status_result = await db.execute(
            select(NotificationLog.delivery_status, func.count(NotificationLog.id))
