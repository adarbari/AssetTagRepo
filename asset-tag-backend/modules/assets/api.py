...
from modules.assets.schemas import AssetCreate, AssetResponse, AssetUpdate
...
def asset_to_response(asset: Asset) -> AssetResponse:
    ...
    return AssetResponse(
        id=(
            str(asset.id)
            if "-" in str(asset.id)
            else f"{str(asset.id)[:8]}-{str(asset.id)[8:12]}-{str(asset.id)[12:16]}-{str(asset.id)[16:20]}-{str(asset.id)[20:]}"
        ),
        organization_id=(
            str(asset.organization_id)
            if "-" in str(asset.organization_id)
            else f"{str(asset.organization_id)[:8]}-{str(asset.organization_id)[8:12]}-{str(asset.organization_id)[12:16]}-{str(asset.organization_id)[16:20]}-{str(asset.organization_id)[20:]}"
        ),
        name=asset.name,
        ...
        status=asset.status,
        current_site_id=(
            str(asset.current_site_id)
            if asset.current_site_id and "-" in str(asset.current_site_id)
            else (
                f"{str(asset.current_site_id)[:8]}-{str(asset.current_site_id)[8:12]}-{str(asset.current_site_id)[12:16]}-{str(asset.current_site_id)[16:20]}-{str(asset.current_site_id)[20:]}"
        ...
        last_seen=(
            ...
            if asset.last_seen and hasattr(asset.last_seen, "isoformat")
            else asset.last_seen
        ),
        ...
        assigned_to_user_id=(
            ...
            if asset.assigned_to_user_id and "-" in str(asset.assigned_to_user_id)
            else (
                f"{str(asset.assigned_to_user_id)[:8]}-{str(asset.assigned_to_user_id)[8:12]}-{str(asset.assigned_to_user_id)[12:16]}-{str(asset.assigned_to_user_id)[16:20]}-{str(asset.assigned_to_user_id)[20:]}"
                if asset.assigned_to_user_id
                else None
            )
        ),
        assigned_job_id=(
            str(asset.assigned_job_id)
            if asset.assigned_job_id and "-" in str(asset.assigned_job_id)
            else (
                f"{str(asset.assigned_job_id)[:8]}-{str(asset.assigned_job_id)[8:12]}-{str(asset.assigned_job_id)[12:16]}-{str(asset.assigned_job_id)[16:20]}-{str(asset.assigned_job_id)[20:]}"
                if asset.assigned_job_id
                else None
            )
        ),
        assignment_start_date=(
            asset.assignment_start_date.isoformat()
            if asset.assignment_start_date
            and hasattr(asset.assignment_start_date, "isoformat")
            else asset.assignment_start_date
        ),
        assignment_end_date=(
            asset.assignment_end_date.isoformat()
            if asset.assignment_end_date
            and hasattr(asset.assignment_end_date, "isoformat")
            else asset.assignment_end_date
        ),
        manufacturer=asset.manufacturer,
        model=asset.model,
        purchase_date=(
            asset.purchase_date.isoformat()
            if asset.purchase_date and hasattr(asset.purchase_date, "isoformat")
            else asset.purchase_date
        ),
        warranty_expiry=(
            asset.warranty_expiry.isoformat()
            if asset.warranty_expiry and hasattr(asset.warranty_expiry, "isoformat")
            else asset.warranty_expiry
        ),
        last_maintenance=(
            asset.last_maintenance.isoformat()
            if asset.last_maintenance and hasattr(asset.last_maintenance, "isoformat")
            else asset.last_maintenance
        ),
        next_maintenance=(
            asset.next_maintenance.isoformat()
            if asset.next_maintenance and hasattr(asset.next_maintenance, "isoformat")
            else asset.next_maintenance
        ),
        hourly_rate=asset.hourly_rate,
        availability=asset.availability,
        asset_metadata=(
            json.loads(asset.asset_metadata)
...
async def create_asset(asset_data: AssetCreate, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        asset = Asset(
            ...
            current_site_id=(
                uuid.UUID(asset_data.current_site_id)
...
async def get_asset_current_location(asset_id: str, db: AsyncSession = Depends(get_db)):
