...
from modules.issues.models import Issue, IssueComment, IssueAttachment
from modules.issues.schemas import (
    ...
    IssueAttachmentCreate, IssueAttachmentResponse,
    IssueWithDetails, IssueStats
)
...
async def get_issue(
    ...
    try:
        result = await db.execute(
            select(Issue).where(
                and_(Issue.id == issue_id, Issue.deleted_at.is_(None))
            )
        )
...
async def get_issue_comments(
    ...
    try:
        ...
        # Get comments
        query = select(IssueComment).where(
            IssueComment.issue_id == issue_id
        ).order_by(IssueComment.created_at.desc()).offset(skip).limit(limit)
...
async def get_issue_stats(
    ...
    try:
        ...
        high_priority_result = await db.execute(
            select(func.count(Issue.id)).where(
                and_(Issue.severity.in_(['high', 'critical']), Issue.deleted_at.is_(None))
            )
        )
