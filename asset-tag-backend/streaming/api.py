...
from streaming.stream_processor_coordinator import get_stream_coordinator, StreamProcessorCoordinator
...
async def get_processor_stats(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting processor stats: {str(e)}")
...
async def restart_processor(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to restart processor {processor_name}")
        
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error restarting processor: {str(e)}")


@router.put("/streaming/processors/anomaly/config")
async def update_anomaly_processor_config(
    anomaly_threshold: float = Query(..., ge=0.0, le=1.0, description="New anomaly threshold"),
    coordinator: StreamProcessorCoordinator = Depends(get_stream_coordinator)
):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(status_code=500, detail="Failed to update anomaly processor configuration")
        
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error updating processor config: {str(e)}")
...
async def force_anomaly_detection(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error forcing anomaly detection: {str(e)}")
...
async def force_geofence_evaluation(
    ...
    try:
        ...
        result = await geofence_processor.force_geofence_evaluation(asset_id, latitude, longitude)
        ...
        raise HTTPException(status_code=500, detail=f"Error forcing geofence evaluation: {str(e)}")
