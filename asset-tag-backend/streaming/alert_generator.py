...
class AlertGenerator:
        ...
        try:
            async for db in get_db():
                alert = Alert(
                    ...
                    asset_name=alert_data.get('asset_name', f"Asset {alert_data['asset_id']}"),
                ...
                logger.info(f"Created alert {alert.id} for asset {alert_data['asset_id']}")
                break
        ...
        try:
            async for db in get_db():
                result = await db.execute(
                    db.query(Alert).where(Alert.id == alert_id)
                )
    ...
    async def resolve_alert(self, alert_id: str, user_id: str = None, notes: str = None):
        """Resolve an alert"""
        try:
            async for db in get_db():
                result = await db.execute(
