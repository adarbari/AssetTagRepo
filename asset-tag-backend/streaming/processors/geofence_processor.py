...
class GeofenceProcessor:
    ...
    def __init__(self, processing_interval: float = 5.0):
        ...
        self.asset_geofence_states = defaultdict(set)  # Track which geofences each asset is in
    ...
    async def process_location_update(self, location_data: Dict[str, Any]):
        ...
        try:
            ...
            logger.error(f"Error processing location update for geofence evaluation: {e}")
    ...
    async def _update_geofence_cache(self):
        ...
        try:
            ...
            logger.info(f"Updated geofence cache with {len(self.geofence_cache)} geofences")
            
        except Exception as e:
            logger.error(f"Error updating geofence cache: {e}")
    
    async def _evaluate_geofences(self, asset_id: str, latitude: float, longitude: float):
        ...
        try:
            ...
            for geofence_id in entries:
                await self._handle_geofence_entry(asset_id, geofence_id, latitude, longitude)
            
            # Process exits
            for geofence_id in exits:
                await self._handle_geofence_exit(asset_id, geofence_id, latitude, longitude)
            
            # Update state
            self.asset_geofence_states[asset_id] = current_geofences
            
            if entries or exits:
                logger.debug(f"Asset {asset_id}: entries={len(entries)}, exits={len(exits)}")
            
        except Exception as e:
            logger.error(f"Error evaluating geofences for asset {asset_id}: {e}")
    
    async def _is_location_in_geofence(self, latitude: float, longitude: float, geofence_data: Dict[str, Any]) -> bool:
    ...
    def _is_in_circle_geofence(self, latitude: float, longitude: float, geofence_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            distance = self._calculate_distance(latitude, longitude, center_lat, center_lng)
    ...
    def _is_in_polygon_geofence(self, latitude: float, longitude: float, geofence_data: Dict[str, Any]) -> bool:
    ...
    async def _handle_geofence_entry(self, asset_id: str, geofence_id: str, latitude: float, longitude: float):
    ...
    async def _handle_geofence_exit(self, asset_id: str, geofence_id: str, latitude: float, longitude: float):
    ...
    async def _create_geofence_alert(self, asset_id: str, geofence_id: str, event_type: str, 
                                   latitude: float, longitude: float, geofence_data: Dict[str, Any]):
    ...
    async def force_geofence_evaluation(self, asset_id: str, latitude: float, longitude: float) -> Dict[str, Any]:
