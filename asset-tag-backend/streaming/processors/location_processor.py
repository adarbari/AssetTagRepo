...
class LocationProcessor:
        ...
        try:
            ...
            async with self.processing_lock:
                self.observation_buffers[asset_id].append(
                    {**observation_data, "received_at": datetime.now()}
                )
    ...
    async def _process_asset_location(self, asset_id: str):
        ...
        try:
            ...
            gateway_observations = await self._convert_to_gateway_observations(
                observations
            )
    ...
    async def _convert_to_gateway_observations(
    ...
    async def _update_location_cache(self, asset_id: str, location):
        ...
        try:
            ...
            await cache.set_with_strategy(
                "asset_location", location_data, asset_id=asset_id
            )
    ...
    async def _trigger_downstream_processing(self, location):
        ...
        try:
            ...
            logger.debug(
                f"Triggering downstream processing for asset {location.asset_id}"
            )
