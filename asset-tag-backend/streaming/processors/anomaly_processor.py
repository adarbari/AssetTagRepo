...
class AnomalyProcessor:
    ...
    def __init__(self, processing_interval: float = 30.0, anomaly_threshold: float = 0.7):
    ...
    async def process_location_update(self, location_data: Dict[str, Any]):
        ...
        try:
            ...
            if self._is_in_cooldown(asset_id):
                logger.debug(f"Asset {asset_id} in cooldown, skipping anomaly detection")
                return
    ...
    async def _detect_anomaly_with_semaphore(self, semaphore: asyncio.Semaphore, asset_id: str):
        """Detect anomaly with semaphore for concurrency control"""
        async with semaphore:
            await self._detect_anomaly(asset_id)
    
    async def _detect_anomaly(self, asset_id: str, location_data: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            if is_anomalous and confidence > 0.5:
                await self._create_anomaly_alert(asset_id, anomaly_score, confidence, prediction)
                self._set_cooldown(asset_id)
            
            logger.debug(f"Anomaly detection for asset {asset_id}: score={anomaly_score:.3f}, anomalous={is_anomalous}")
            
        except Exception as e:
            logger.error(f"Error detecting anomaly for asset {asset_id}: {e}")
    
    async def _create_anomaly_alert(self, asset_id: str, anomaly_score: float, confidence: float, prediction: Dict[str, Any]):
        ...
        try:
            ...
            description = f"Anomaly score: {anomaly_score:.3f}, Confidence: {confidence:.3f}"
            ...
            logger.info(f"Created anomaly alert for asset {asset_id} with score {anomaly_score:.3f}")
    ...
    async def get_processing_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            assets_in_cooldown = sum(1 for cooldown_time in self.asset_cooldowns.values() 
            ...
            return {
                ...
                "cooldown_duration_minutes": self.cooldown_duration.total_seconds() / 60,
