...
class GeofenceEvaluator:
    ...
    def _point_in_geofence(self, point: Point, geofence: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if geofence_type == 'circular':
                ...
                distance = point.distance(center) * 111000  # Convert to meters (rough approximation)
                ...
                polygon_coords = [(coord[1], coord[0]) for coord in coordinates]  # (lng, lat)
    ...
    async def _handle_geofence_entry(self, asset_id: str, geofence: Dict[str, Any], location: EstimatedLocation):
        ...
        try:
            ...
            if geofence.get('alert_on_entry', True):
                await self._trigger_geofence_alert(asset_id, geofence, 'entry', location)
                
        except Exception as e:
            logger.error(f"Error handling geofence entry: {e}")
    
    async def _handle_geofence_exit(self, asset_id: str, geofence: Dict[str, Any], location: EstimatedLocation):
