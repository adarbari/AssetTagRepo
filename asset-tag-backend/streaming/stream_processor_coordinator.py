...
from streaming.processors.location_processor import (
    get_location_processor, start_location_processor, stop_location_processor
)
from streaming.processors.anomaly_processor import (
    get_anomaly_processor, start_anomaly_processor, stop_anomaly_processor
)
from streaming.processors.geofence_processor import (
    get_geofence_processor, start_geofence_processor, stop_geofence_processor
)
...
class StreamProcessorCoordinator:
    ...
    async def update_processor_config(self, processor_name: str, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if processor_name == "anomaly" and "anomaly_threshold" in config:
                ...
                logger.info(f"Updated {processor_name} anomaly threshold to {config['anomaly_threshold']}")
                return True
            else:
                logger.warning(f"No configurable parameters for processor {processor_name}")
                return False
