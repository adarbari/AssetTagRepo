...
export function CreateGeofence({ onBack, onGeofenceCreated, onGeofenceUpdated, editMode = false, existingGeofenceId, initialData }: CreateGeofenceProps) {
  const [name, setName] = useState("");
  const [geofenceType, setGeofenceType] = useState<"authorized" | "restricted">("authorized");
    ...
    if (editMode && existingGeofenceId) {
      ...
      import("../data/mockData").then(({ getGeofenceById }) => {
        ...
        if (geofence) {
          ...
          if (geofence.tolerance !== undefined) setTolerance(geofence.tolerance);
          if (geofence.alertOnEntry !== undefined) setAlertOnEntry(geofence.alertOnEntry);
          if (geofence.alertOnExit !== undefined) setAlertOnExit(geofence.alertOnExit);
        }
      });
    } else if (initialData) {
      ...
      if (initialData.latitude !== undefined || initialData.longitude !== undefined || initialData.radius !== undefined) {
        setCoordinates({
      ...
      if (initialData.tolerance !== undefined) setTolerance(initialData.tolerance);
      if (initialData.alertOnEntry !== undefined) setAlertOnEntry(initialData.alertOnEntry);
      if (initialData.alertOnExit !== undefined) setAlertOnExit(initialData.alertOnExit);
    }
    ...
    if (editMode && existingGeofenceId) {
      ...
      import("../data/mockData").then(({ updateGeofence }) => {
        const updates = {
          ...
          type: shape,
          center: shape === "circular" ? [lat, lng] as [number, number] : undefined,
      ...
      // Create new geofence
      import("../data/mockData").then(({ createGeofence, generateGeofenceId }) => {
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Select value={geofenceType} onValueChange={(v: "authorized" | "restricted") => setGeofenceType(v)}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <p className="text-xs text-muted-foreground">
                  {geofenceType === "authorized" && "Assets are expected to be in this zone"}
                  {geofenceType === "restricted" && "Assets should not enter this zone"}
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select value={shape} onValueChange={(v: "circular" | "polygon") => setShape(v)}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="p-4 bg-muted rounded-lg">
                ...
                <p className="text-sm text-muted-foreground">
                  Additional alert rules such as dwell time alerts, specific asset alerts, and schedule-based
                  notifications can be configured after creating the geofence in the Geofences management page.
                </p>
              </div>
            </CardContent>
          </Card>
