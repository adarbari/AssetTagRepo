...
export function EditMaintenance({ 
  ...
  const loadData = async () => {
    try {
      ...
      const [taskData, types, priorityOptions, statusOptions, technicianOptions] = await Promise.all([
  ...
  const handleSave = async () => {
    try {
      ...
      if (formData.task !== task.task) {
        ...
        changes.push({ field: "Task Description", from: task.task, to: formData.task });
      }
      if (formData.dueDate !== task.dueDate) {
        updates.dueDate = formData.dueDate;
        changes.push({ field: "Due Date", from: task.dueDate, to: formData.dueDate });
      }
      if (formData.priority !== task.priority) {
        updates.priority = formData.priority as any;
        changes.push({ field: "Priority", from: task.priority, to: formData.priority });
      }
      if (formData.status !== task.status) {
        updates.status = formData.status as any;
        changes.push({ field: "Status", from: task.status, to: formData.status });
      }
      if (formData.assignedTo !== task.assignedTo) {
        updates.assignedTo = formData.assignedTo;
        changes.push({ field: "Assigned To", from: task.assignedTo, to: formData.assignedTo });
      }
      if (formData.estimatedDuration !== (task.estimatedDuration?.toString() || "")) {
        updates.estimatedDuration = formData.estimatedDuration ? parseInt(formData.estimatedDuration) : undefined;
        changes.push({ 
      ...
      if (formData.cost !== (task.cost || "")) {
        ...
        changes.push({ field: "Cost", from: task.cost || "-", to: formData.cost || "-" });
      }
      if (formData.notes !== (task.notes || "")) {
        updates.notes = formData.notes;
        changes.push({ field: "Notes", from: task.notes || "-", to: formData.notes || "-" });
      }
