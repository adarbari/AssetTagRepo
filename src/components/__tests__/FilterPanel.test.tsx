...
describe('FilterPanel Component', () => {
  ...
  describe('Filter Popover', () => {
    ...
    it('should show clear all button when filters are active', () => {
      render(<FilterPanel {...defaultProps} showFilters={true} activeFiltersCount={2} />)
      
      expect(screen.getByText('Clear All')).toBeInTheDocument()
    })

    it('should not show clear all button when no filters are active', () => {
      render(<FilterPanel {...defaultProps} showFilters={true} activeFiltersCount={0} />)
  ...
  describe('Badge Interactions', () => {
    ...
    it('should call onValueChange when filter badge X is clicked', async () => {
      ...
      await user.click(xButton!)
      expect(filtersWithActiveValues[0].onValueChange).toHaveBeenCalledWith('all')
    })
  })
