...
vi.mock('../GeofenceMapEditor', () => ({
  GeofenceMapEditor: ({ coordinates, tolerance, name, isEditing, onCoordinatesChange, onToleranceChange }: any) => (
    <div data-testid="geofence-map-editor">
      ...
      <button 
        onClick={() => onCoordinatesChange({ lat: 30.2672, lng: -97.7431, radius: 600 })}
...
describe('SiteDetails Component', () => {
  ...
  describe('Edit Functionality', () => {
    it('should enter edit mode when Edit Site button is clicked', async () => {
      ...
      expect(screen.getByDisplayValue('123 Main St, Austin, TX 78701')).toBeInTheDocument()
    })
  ...
  describe('Location and Boundary Management', () => {
    it('should render geofence map editor in location tab', async () => {
      ...
      expect(screen.getByTestId('geofence-map-editor')).toBeInTheDocument()
      expect(screen.getByText('Map Editor for Main Warehouse')).toBeInTheDocument()
    })
    ...
    it('should enter boundary editing mode when Edit Boundary button is clicked', async () => {
      ...
      const editBoundaryButton = screen.getByRole('button', { name: /edit boundary/i })
    ...
    it('should update coordinates when map editor calls onCoordinatesChange', async () => {
      ...
      expect(screen.getByText('Coordinates: 30.2672, -97.7431')).toBeInTheDocument()
    })
  ...
  describe('Geofence Management', () => {
    ...
    it('should show create geofence button when no geofence exists', async () => {
      ...
      expect(screen.getByText('No geofence configured for this site')).toBeInTheDocument()
      expect(screen.getByRole('button', { name: /create geofence/i })).toBeInTheDocument()
    })
    ...
    it('should call onCreateGeofence when Create Geofence button is clicked', async () => {
      ...
      const createButton = screen.getByRole('button', { name: /create geofence/i })
    ...
    it('should call onEditGeofence when Edit button is clicked on existing geofence', async () => {
      ...
      expect(defaultProps.onEditGeofence).toHaveBeenCalledWith('GF-001', {
        siteId: 'ST-001',
        siteName: 'Main Warehouse',
        latitude: 30.2672,
        longitude: -97.7431,
  ...
  describe('Accessibility', () => {
    ...
    it('should maintain keyboard navigation', async () => {
      ...
      await user.tab()
      expect(document.activeElement).toBe(screen.getByRole('button', { name: '' })) // Back button
      
      await user.tab()
      expect(document.activeElement).toBe(screen.getByRole('button', { name: /edit site/i }))
    })
  })
