...
describe('ComplianceTracking', () => {
  ...
  describe('Component Rendering', () => {
    it('renders the compliance tracking page with correct title and description', () => {
      ...
      expect(screen.getByText('Manage certifications, inspections, and regulatory compliance')).toBeInTheDocument()
    })
  ...
  describe('Search Functionality', () => {
    it('filters records based on search term', () => {
      ...
      const searchInput = screen.getByPlaceholderText('Search assets or certificates...')
  ...
  describe('User Interactions', () => {
    ...
    it('shows toast message when Download Certificate is clicked', () => {
      ...
      const moreButton = actionButtons.find(button => 
        button.querySelector('svg') // Look for the MoreVertical icon
      )
      ...
      if (moreButton) {
        ...
        expect(toast.success).toHaveBeenCalledWith('Downloading certificate for Generator-001')
      }
    })
    ...
    it('shows toast message when Renew Certificate is clicked for expiring/expired records', () => {
      ...
      if (moreButtons.length > 1) {
        ...
        expect(toast.success).toHaveBeenCalledWith('Renewal process initiated for Load Test')
      }
    })
  })
  ...
  describe('Empty States', () => {
    it('shows empty state when no records are found', () => {
      ...
      expect(screen.getByText('No compliance records found')).toBeInTheDocument()
      expect(screen.getByText('No compliance records in this category')).toBeInTheDocument()
