...
interface JobManagementProps {
  ...
  onCreateJob: (input: CreateJobInput) => Promise<{ success: boolean; job?: Job; error?: any }>;
...
export function JobManagement({
  ...
  const filteredJobs = jobsList.filter(job => {
    const matchesSearch = job.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         ...
                         (job.projectManager && job.projectManager.toLowerCase().includes(searchQuery.toLowerCase())) ||
                         (job.siteName && job.siteName.toLowerCase().includes(searchQuery.toLowerCase()));
                  ...
                  <TableRow key={job.id} className="cursor-pointer hover:bg-muted/50"
                    ...
                    <TableCell>
                      <div>
                        <div>{job.name}</div>
                        {job.siteName && (
                          <div className="text-xs text-muted-foreground">{job.siteName}</div>
                        )}
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>{formatCurrency(job.actualCosts.total)}</TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <span className={job.variancePercentage >= 0 ? "text-green-600" : "text-red-600"}>
