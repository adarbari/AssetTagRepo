...
class TestRunner {
  ...
  async runAllTests(): Promise<void> {
    console.log('🚀 Starting comprehensive test suite for AssetTag Application\n')
  ...
  private printSummary(): void {
    ...
    Object.entries(componentTests).forEach(([component, tests]) => {
      ...
      const status = componentTotal === 0 ? '⏭️' : componentPassed === componentTotal ? '✅' : '❌'
    ...
    console.log(`Average Test Duration: ${(this.results.reduce((sum, r) => sum + (r.duration || 0), 0) / total).toFixed(0)}ms`)
