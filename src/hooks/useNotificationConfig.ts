...
export function useNotificationConfig() {
  const [configs, setConfigs] = useState<Record<string, NotificationPreferences>>({});
  ...
  const loadConfigs = () => {
    try {
      ...
      if (saved) {
        ...
        localStorage.setItem("notificationConfigurations", JSON.stringify(initialConfigs));
      }
    } catch (error) {
  ...
  const getConfig = (level: string, entityId: string): NotificationPreferences | undefined => {
    const key = `${level}:${entityId}`;
    return configs[key];
  };
