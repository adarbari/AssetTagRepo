import type { Asset, Site, Alert, MaintenanceRecord, Geofence, Personnel, ActivityEvent } from "../types";
...
export const mockAssets: Asset[] = [
  ...
  },
  {
    ...
    coordinates: [37.7450, -122.4500], // Outside GEO-004 boundary (125ft radius)
    temperature: 65,
    movement: "stationary",
    hourlyRate: 35.00,
    availability: "available",
  },
  {
    ...
    coordinates: [37.7900, -122.4050], // Outside GEO-001 boundary
    temperature: 73,
    movement: "stationary",
  },
  {
    ...
    coordinates: [37.7750, -122.4000], // Outside GEO-002 boundary (100ft radius)
    temperature: 66,
    movement: "stationary",
  },
];
...
export const mockAlerts: Alert[] = [
  ...
  // Offline/Not Reachable Alerts  
  {
    ...
    metadata: {
      ...
      lastBatteryLevel: 45,
      possibleCauses: ["Dead tracker battery", "Signal obstruction", "Device tampered"]
    }
  },
  {
    id: "ALT-1010",
  ...
  },
  {
    ...
    metadata: {
      ...
      normalTemp: 70,
      threshold: 75,
      possibleIssues: ["Low hydraulic fluid", "Cooling system failure", "Pump malfunction"],
...
// In production, this would be an API call
export function updateGeofence(geofenceId: string, updates: Partial<Geofence>): Geofence {
  const geofenceIndex = mockGeofences.findIndex(g => g.id === geofenceId);
  if (geofenceIndex === -1) {
    throw new Error(`Geofence ${geofenceId} not found`);
  }
  
  mockGeofences[geofenceIndex] = { ...mockGeofences[geofenceIndex], ...updates };
...
function generateActivityHistory(entityId: string, currentSiteId?: string): ActivityEvent[] {
  ...
  for (let day = 30; day >= 0; day--) {
    ...
    for (let visit = 0; visit < visitsToday; visit++) {
      ...
      departure.setHours(arrival.getHours() + durationHours, Math.floor(Math.random() * 60), 0, 0);
...
// In production, this would be an API call
export function updateMockAsset(assetId: string, updates: Partial<Asset>): Asset | null {
  const assetIndex = mockAssets.findIndex(a => a.id === assetId);
...
function isPointInCircle(point: [number, number], center: [number, number], radiusInFeet: number): boolean {
...
export function getGeofenceViolatingAssets(geofenceId: string): Asset[] {
  ...
  if (!geofence || !geofence.expectedAssetIds || geofence.expectedAssetIds.length === 0) {
    return [];
  }
  ...
  for (const assetId of geofence.expectedAssetIds) {
    ...
    if (geofence.type === "circular" && geofence.center && geofence.radius) {
      isInside = isPointInCircle(asset.coordinates, geofence.center, geofence.radius);
...
export function getGeofenceExpectedAssets(geofenceId: string): Asset[] {
  ...
  return mockAssets.filter(asset => geofence.expectedAssetIds!.includes(asset.id));
}
...
export function getGeofenceComplianceStats(geofenceId: string): {
  ...
  const complianceRate = expected > 0 ? Math.round((inside / expected) * 100) : 100;
